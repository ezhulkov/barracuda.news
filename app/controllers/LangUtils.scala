...
object LangUtils {
  ...
  def addLang(url: String)(implicit env: Environment, lang: LanguageValue): String = {
    val langCode = lang match {
      case Language.DEFAULT => None
      case _ => Some(lang)
    }
    env.mode match {
      case Mode.Prod => langCode.map(t => s"${t.code}/$url").getOrElse(url)
      case _ => langCode.map(t => s"$url?lang=${t.code}").getOrElse(url)
    }
  }
